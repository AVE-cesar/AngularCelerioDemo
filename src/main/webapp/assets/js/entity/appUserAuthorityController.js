...
// Template angular-lab:angularjs/static/assets/js/entity/EntityController.e.vm.js
//


app.controller("AppUserAuthorityController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "AppUserAuthorityRestService", "AppUserAuthorityRestSearchService", 
		"AppUserAuthorityRestIndexService", "AppUserAuthorityRestMassDeleteService",
					"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		appUserAuthorityRestService, appUserAuthorityRestSearchService, appUserAuthorityRestIndexService, 
		appUserAuthorityRestMassDeleteService, 
					alertService, timeoutService, config) {

log.info("inside AppUserAuthorityController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside AppUserAuthorityController");
	appUserAuthorityRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside AppUserAuthorityController");
	appUserAuthorityRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	appUserAuthorityRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside AppUserAuthorityController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside AppUserAuthorityController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside AppUserAuthorityController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside AppUserAuthorityController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside AppUserAuthorityController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside AppUserAuthorityController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	appUserAuthorityRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	appUserAuthorityRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-appUserAuthority.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/appUserAuthority/appUserAuthoritySearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	appUserAuthorityRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your appUserAuthority have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your appUserAuthority haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		appUserAuthorityRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		appUserAuthorityRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			appUserAuthorityRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your appUserAuthority have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			appUserAuthorityRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	appUserAuthorityRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) AppUserAuthority entity */
app.factory('AppUserAuthorityRestService', function ($resource) {
	return $resource('api/appUserAuthorities/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/appUserAuthorities/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/appUserAuthorities/:id'},
		'update': { method:'PUT', url: 'api/appUserAuthorities/:id'},
		'delete': { method:'DELETE', url: 'api/appUserAuthorities/:id' },
		'search': { method: 'POST', url: 'api/appUserAuthorities/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on AppUserAuthority entity */
app.factory('AppUserAuthorityRestSearchService', function ($resource) {
	return $resource('api/appUserAuthorities/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing AppUserAuthority entity */
app.factory('AppUserAuthorityRestIndexService', function ($resource) {
	return $resource('api/appUserAuthorities/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on AppUserAuthority entity */
app.factory('AppUserAuthorityRestMassDeleteService', function ($resource) {
	return $resource('api/appUserAuthorities/mass/:id', {}, {
...
app.factory('AppUserAuthorityRestInvRelationService', function ($resource) {
