...
app.controller("AppTokenController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "AppTokenRestService", "AppTokenRestSearchService", 
		"AppTokenRestIndexService", "AppTokenRestMassDeleteService",
								"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		appTokenRestService, appTokenRestSearchService, appTokenRestIndexService, 
		appTokenRestMassDeleteService, 
								alertService, timeoutService, config) {

log.info("inside AppTokenController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside AppTokenController");
	appTokenRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside AppTokenController");
	appTokenRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	appTokenRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside AppTokenController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside AppTokenController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside AppTokenController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside AppTokenController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside AppTokenController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside AppTokenController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	appTokenRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	appTokenRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-appToken.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/appToken/appTokenSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	appTokenRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your appToken have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your appToken haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		appTokenRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		appTokenRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			appTokenRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your appToken have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			appTokenRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	appTokenRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) AppToken entity */
app.factory('AppTokenRestService', function ($resource) {
	return $resource('api/appTokens/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/appTokens/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/appTokens/:id'},
		'update': { method:'PUT', url: 'api/appTokens/:id'},
		'delete': { method:'DELETE', url: 'api/appTokens/:id' },
		'search': { method: 'POST', url: 'api/appTokens/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on AppToken entity */
app.factory('AppTokenRestSearchService', function ($resource) {
	return $resource('api/appTokens/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing AppToken entity */
app.factory('AppTokenRestIndexService', function ($resource) {
	return $resource('api/appTokens/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on AppToken entity */
app.factory('AppTokenRestMassDeleteService', function ($resource) {
	return $resource('api/appTokens/mass/:id', {}, {
...
app.factory('AppTokenRestInvRelationService', function ($resource) {
