...
app.controller("AppUserController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "AppUserRestService", "AppUserRestSearchService", 
		"AppUserRestIndexService", "AppUserRestMassDeleteService",
										"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		appUserRestService, appUserRestSearchService, appUserRestIndexService, 
		appUserRestMassDeleteService, 
										alertService, timeoutService, config) {

log.info("inside AppUserController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside AppUserController");
	appUserRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside AppUserController");
	appUserRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	appUserRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside AppUserController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside AppUserController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside AppUserController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside AppUserController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside AppUserController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside AppUserController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	appUserRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	appUserRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-appUser.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/appUser/appUserSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	appUserRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your appUser have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your appUser haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		appUserRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		appUserRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			appUserRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your appUser have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			appUserRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) AppUser entity */
app.factory('AppUserRestService', function ($resource) {
	return $resource('api/appUsers/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/appUsers/:id'},
		'update': { method:'PUT', url: 'api/appUsers/:id'},
		'delete': { method:'DELETE', url: 'api/appUsers/:id' },
		'search': { method: 'POST', url: 'api/appUsers/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on AppUser entity */
app.factory('AppUserRestSearchService', function ($resource) {
	return $resource('api/appUsers/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing AppUser entity */
app.factory('AppUserRestIndexService', function ($resource) {
	return $resource('api/appUsers/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on AppUser entity */
app.factory('AppUserRestMassDeleteService', function ($resource) {
	return $resource('api/appUsers/mass/:id', {}, {
		'massDelete': { method: 'DELETE'}
	});
});


	/** REST client for managing linked entities for AppUser entity */
	app.factory('AppUserLinkedEntitiesService', function ($resource) {
								return $resource('api/appAuthorities/appAuthoritiesByAppUsersId/:id', {}, {
	        'queryAppAuthoritiesByAppUsersId': {
	        	method: 'GET',
	        	url: 'api/appAuthorities/appAuthoritiesByAppUsersId/:id',
	        	isArray: true
	        }
				    })});

/** REST client for managing inverse relation */
app.factory('AppUserRestInvRelationService', function ($resource) {
