...
app.controller("BookController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
...
scope.refresh = function () {
	log.info("call method refresh inside BookController");
	bookRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside BookController");
	bookRestService.query({page: page, size: size}, function(result) {
...
scope.first = function () {
	log.info("call method first inside BookController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside BookController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside BookController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside BookController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside BookController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside BookController");
	scope.savedSearch = {};
};
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/book/bookSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
app.factory('BookRestService', function ($resource) {
	return $resource('api/books/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/books/:id'},
		'update': { method:'PUT', url: 'api/books/:id'},
...
app.factory('BookRestSearchService', function ($resource) {
	return $resource('api/books/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Book entity */
app.factory('BookRestIndexService', function ($resource) {
	return $resource('api/books/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Book entity */
app.factory('BookRestMassDeleteService', function ($resource) {
	return $resource('api/books/mass/:id', {}, {
...
app.factory('BookRestInvRelationService', function ($resource) {
