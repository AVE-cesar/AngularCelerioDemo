...
app.controller("AppAuthorityController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "AppAuthorityRestService", "AppAuthorityRestSearchService", 
		"AppAuthorityRestIndexService", "AppAuthorityRestMassDeleteService",
				"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		appAuthorityRestService, appAuthorityRestSearchService, appAuthorityRestIndexService, 
		appAuthorityRestMassDeleteService, 
				alertService, timeoutService, config) {

log.info("inside AppAuthorityController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside AppAuthorityController");
	appAuthorityRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside AppAuthorityController");
	appAuthorityRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	appAuthorityRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside AppAuthorityController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside AppAuthorityController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside AppAuthorityController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside AppAuthorityController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside AppAuthorityController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside AppAuthorityController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	appAuthorityRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	appAuthorityRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-appAuthority.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/appAuthority/appAuthoritySearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	appAuthorityRestService.get({id: item.id}, function success(result) {
		scope.item = result;
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your appAuthority have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			appAuthorityRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your appAuthority have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			appAuthorityRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	appAuthorityRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) AppAuthority entity */
app.factory('AppAuthorityRestService', function ($resource) {
	return $resource('api/appAuthorities/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/appAuthorities/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/appAuthorities/:id'},
		'update': { method:'PUT', url: 'api/appAuthorities/:id'},
		'delete': { method:'DELETE', url: 'api/appAuthorities/:id' },
		'search': { method: 'POST', url: 'api/appAuthorities/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on AppAuthority entity */
app.factory('AppAuthorityRestSearchService', function ($resource) {
	return $resource('api/appAuthorities/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing AppAuthority entity */
app.factory('AppAuthorityRestIndexService', function ($resource) {
	return $resource('api/appAuthorities/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on AppAuthority entity */
app.factory('AppAuthorityRestMassDeleteService', function ($resource) {
	return $resource('api/appAuthorities/mass/:id', {}, {
...
app.factory('AppAuthorityRestInvRelationService', function ($resource) {
	return $resource('api/void/:id', {}, {
		'void': { method: 'GET'} /* dummy method, to be sure to have at least one */

             ,'findAppUserByAppAuthority': { method: 'GET', isArray: true, url: 'api/appUsers/findByAppAuthority/:id'}
    	});
});
