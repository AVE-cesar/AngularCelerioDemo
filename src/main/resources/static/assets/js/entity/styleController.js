...
app.controller("StyleController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "StyleRestService", "StyleRestSearchService", 
		"StyleRestIndexService", "StyleRestMassDeleteService",
				"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		styleRestService, styleRestSearchService, styleRestIndexService, 
		styleRestMassDeleteService, 
				alertService, timeoutService, config) {

log.info("inside StyleController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside StyleController");
	styleRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside StyleController");
	styleRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	styleRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside StyleController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside StyleController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside StyleController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside StyleController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside StyleController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside StyleController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	styleRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	styleRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-style.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/style/styleSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	styleRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your style have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your style haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		styleRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		styleRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			styleRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your style have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			styleRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Style entity */
app.factory('StyleRestService', function ($resource) {
	return $resource('api/styles/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/styles/:id'},
		'update': { method:'PUT', url: 'api/styles/:id'},
		'delete': { method:'DELETE', url: 'api/styles/:id' },
		'search': { method: 'POST', url: 'api/styles/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on Style entity */
app.factory('StyleRestSearchService', function ($resource) {
	return $resource('api/styles/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Style entity */
app.factory('StyleRestIndexService', function ($resource) {
	return $resource('api/styles/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Style entity */
app.factory('StyleRestMassDeleteService', function ($resource) {
	return $resource('api/styles/mass/:id', {}, {
...
app.factory('StyleRestInvRelationService', function ($resource) {
	return $resource('api/void/:id', {}, {
		'void': { method: 'GET'} /* dummy method, to be sure to have at least one */

              ,'findAuthorStyleByStyle': { method: 'GET', isArray: true, url: 'api/authorStyles/findByStyle/:id'}
           	});
});
