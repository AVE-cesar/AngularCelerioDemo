...
app.controller("StoreController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "StoreRestService", "StoreRestSearchService", 
		"StoreRestIndexService", "StoreRestMassDeleteService",
							"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		storeRestService, storeRestSearchService, storeRestIndexService, 
		storeRestMassDeleteService, 
							alertService, timeoutService, config) {

log.info("inside StoreController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside StoreController");
	storeRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside StoreController");
	storeRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	storeRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside StoreController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside StoreController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside StoreController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside StoreController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside StoreController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside StoreController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	storeRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	storeRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-store.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/store/storeSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	storeRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	if (scope.item.id != null) {
		// update mode
		storeRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		storeRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			storeRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your store have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			storeRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Store entity */
app.factory('StoreRestService', function ($resource) {
	return $resource('api/stores/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/stores/:id'},
		'update': { method:'PUT', url: 'api/stores/:id'},
		'delete': { method:'DELETE', url: 'api/stores/:id' },
		'search': { method: 'POST', url: 'api/stores/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on Store entity */
app.factory('StoreRestSearchService', function ($resource) {
	return $resource('api/stores/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Store entity */
app.factory('StoreRestIndexService', function ($resource) {
	return $resource('api/stores/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Store entity */
app.factory('StoreRestMassDeleteService', function ($resource) {
	return $resource('api/stores/mass/:id', {}, {
...
app.factory('StoreRestInvRelationService', function ($resource) {
	return $resource('api/void/:id', {}, {
		'void': { method: 'GET'} /* dummy method, to be sure to have at least one */

        ,'findBookByStore': { method: 'GET', isArray: true, url: 'api/books/findByStore/:id'}
              	});
});
