...
app.controller("AddressController", ["$scope", "$window", "$aside", 
"$log", "AddressRestService", "AddressRestSearchService", 
		"AddressRestIndexService", "AddressRestMassDeleteService",
					"$alert", "$timeout", "config", function(scope, window, c, log, 
		addressRestService, addressRestSearchService, addressRestIndexService, 
		addressRestMassDeleteService, 
					alertService, timeoutService, config) {

log.info("inside AddressController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside AddressController");
	addressRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside AddressController");
	addressRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	addressRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside AddressController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside AddressController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside AddressController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside AddressController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside AddressController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside AddressController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	addressRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	addressRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-address.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/address/addressSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	addressRestService.get({id: item.id}, function success(result) {
		scope.item = result;
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your address have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			addressRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your address have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			addressRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Address entity */
app.factory('AddressRestService', function ($resource) {
	return $resource('api/addresses/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/addresses/:id'},
		'update': { method:'PUT', url: 'api/addresses/:id'},
		'delete': { method:'DELETE', url: 'api/addresses/:id' },
		'search': { method: 'POST', url: 'api/addresses/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on Address entity */
app.factory('AddressRestSearchService', function ($resource) {
	return $resource('api/addresses/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Address entity */
app.factory('AddressRestIndexService', function ($resource) {
	return $resource('api/addresses/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Address entity */
app.factory('AddressRestMassDeleteService', function ($resource) {
	return $resource('api/addresses/mass/:id', {}, {
...
app.factory('AddressRestInvRelationService', function ($resource) {
