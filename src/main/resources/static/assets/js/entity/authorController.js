...
app.controller("AuthorController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "AuthorRestService", "AuthorRestSearchService", 
		"AuthorRestIndexService", "AuthorRestMassDeleteService",
					"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		authorRestService, authorRestSearchService, authorRestIndexService, 
		authorRestMassDeleteService, 
					alertService, timeoutService, config) {

log.info("inside AuthorController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside AuthorController");
	authorRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside AuthorController");
	authorRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	authorRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside AuthorController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside AuthorController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside AuthorController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside AuthorController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside AuthorController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside AuthorController");
	scope.savedSearch = {};
};
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/author/authorSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your author have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your author haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			authorRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your author have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Author entity */
app.factory('AuthorRestService', function ($resource) {
	return $resource('api/authors/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/authors/:id'},
		'update': { method:'PUT', url: 'api/authors/:id'},
		'delete': { method:'DELETE', url: 'api/authors/:id' },
		'search': { method: 'POST', url: 'api/authors/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on Author entity */
app.factory('AuthorRestSearchService', function ($resource) {
	return $resource('api/authors/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Author entity */
app.factory('AuthorRestIndexService', function ($resource) {
	return $resource('api/authors/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Author entity */
app.factory('AuthorRestMassDeleteService', function ($resource) {
	return $resource('api/authors/mass/:id', {}, {
...
app.factory('AuthorRestInvRelationService', function ($resource) {
	return $resource('api/void/:id', {}, {
		'void': { method: 'GET'} /* dummy method, to be sure to have at least one */

        ,'findBookByAuthor': { method: 'GET', isArray: true, url: 'api/books/findByAuthor/:id'}
              	});
});
