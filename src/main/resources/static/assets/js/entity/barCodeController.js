...
app.controller("BarCodeController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "BarCodeRestService", "BarCodeRestSearchService", 
		"BarCodeRestIndexService", "BarCodeRestMassDeleteService",
					"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		barCodeRestService, barCodeRestSearchService, barCodeRestIndexService, 
		barCodeRestMassDeleteService, 
					alertService, timeoutService, config) {

log.info("inside BarCodeController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside BarCodeController");
	barCodeRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside BarCodeController");
	barCodeRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	barCodeRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside BarCodeController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside BarCodeController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside BarCodeController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside BarCodeController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside BarCodeController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside BarCodeController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	barCodeRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	barCodeRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-barCode.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/barCode/barCodeSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	barCodeRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your barCode have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your barCode haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		barCodeRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		barCodeRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			barCodeRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your barCode have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			barCodeRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) BarCode entity */
app.factory('BarCodeRestService', function ($resource) {
	return $resource('api/barCodes/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/barCodes/:id'},
		'update': { method:'PUT', url: 'api/barCodes/:id'},
		'delete': { method:'DELETE', url: 'api/barCodes/:id' },
		'search': { method: 'POST', url: 'api/barCodes/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on BarCode entity */
app.factory('BarCodeRestSearchService', function ($resource) {
	return $resource('api/barCodes/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing BarCode entity */
app.factory('BarCodeRestIndexService', function ($resource) {
	return $resource('api/barCodes/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on BarCode entity */
app.factory('BarCodeRestMassDeleteService', function ($resource) {
	return $resource('api/barCodes/mass/:id', {}, {
...
app.factory('BarCodeRestInvRelationService', function ($resource) {
	return $resource('api/void/:id', {}, {
		'void': { method: 'GET'} /* dummy method, to be sure to have at least one */

        ,'findBookByBarCode': { method: 'GET', isArray: true, url: 'api/books/findByBarCode/:id'}
              	});
});
