...
app.controller("ActorController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "ActorRestService", "ActorRestSearchService", 
		"ActorRestIndexService", "ActorRestMassDeleteService",
														"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		actorRestService, actorRestSearchService, actorRestIndexService, 
		actorRestMassDeleteService, 
														alertService, timeoutService, config) {

log.info("inside ActorController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside ActorController");
	actorRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside ActorController");
	actorRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	actorRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside ActorController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside ActorController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside ActorController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside ActorController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside ActorController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside ActorController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	actorRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	actorRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-actor.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/actor/actorSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	actorRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your actor have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your actor haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		actorRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		actorRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			actorRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your actor have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			actorRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Actor entity */
app.factory('ActorRestService', function ($resource) {
	return $resource('api/actors/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/actors/:id'},
		'update': { method:'PUT', url: 'api/actors/:id'},
		'delete': { method:'DELETE', url: 'api/actors/:id' },
		'search': { method: 'POST', url: 'api/actors/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on Actor entity */
app.factory('ActorRestSearchService', function ($resource) {
	return $resource('api/actors/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Actor entity */
app.factory('ActorRestIndexService', function ($resource) {
	return $resource('api/actors/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Actor entity */
app.factory('ActorRestMassDeleteService', function ($resource) {
	return $resource('api/actors/mass/:id', {}, {
...
app.factory('ActorRestInvRelationService', function ($resource) {
