...
app.controller("ReaderEditController", ["$scope", "$window", "$aside", 
"$log", "ReaderRestService", 
		"ReaderRestInvRelationService", "$alert", "$timeout", "item", "mode", function(scope, window, aside, log, 
		readerRestService, 
					readerRestInvRelationService, alertService, timeoutService, item, mode) {
	
	log.info("inside ReaderEditController, mode: " + mode);
	log.info("inside ReaderEditController, item: " + item);
	scope.mode = mode;
	scope.item = item;

				
	readerRestInvRelationService.findBookByReader({id: item.id}, function(result) {
	    scope.findBookByReader = result;
	    log.info("inv. relation, Book data post refresh: " + scope.findBookByReader.length);
    });
		...
		if (r == true) {
			if (item) {				
				// one item deletion mode
												readerRestService.delete({id: item.id}, function success(data) {
					scope.clear();
					alert('item deleted');
