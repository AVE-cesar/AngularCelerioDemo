...
app.controller("SampleEditController", ["$scope", "$window", "$aside", 
"$log", "SampleRestService", 
		"SampleRestInvRelationService", "$alert", "$timeout", "item", "mode", function(scope, window, aside, log, 
		sampleRestService, 
				sampleRestInvRelationService, alertService, timeoutService, item, mode) {
	
	log.info("inside SampleEditController, mode: " + mode);
	log.info("inside SampleEditController, item: " + item);
	scope.mode = mode;
	scope.item = item;
		...
		var onSaveSuccess = function success(data) {
			...
			window.showAlert = function(){
				...
				var userALert = alertService({
		                    title: "SUCCESS:",
		                    content: '<BR>Your sample have been <i>created or updated</i>. You can find it in the result table. See <a href="#"><B>older messages</B></a> !',
		                    placement: "top-right",
		                    type: "theme",
		...
		if (r == true) {
			if (item) {				
				// one item deletion mode
												sampleRestService.delete({id: item.id}, function success(data) {
					scope.clear();
					alert('item deleted');
