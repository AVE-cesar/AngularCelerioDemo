...
app.controller("DocumentsController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "DocumentsRestService", "DocumentsRestSearchService", 
		"DocumentsRestIndexService", "DocumentsRestMassDeleteService",
				"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		documentsRestService, documentsRestSearchService, documentsRestIndexService, 
		documentsRestMassDeleteService, 
				alertService, timeoutService, config) {

log.info("inside DocumentsController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside DocumentsController");
	documentsRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside DocumentsController");
	documentsRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	documentsRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside DocumentsController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside DocumentsController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside DocumentsController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside DocumentsController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside DocumentsController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside DocumentsController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	documentsRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	documentsRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-documents.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/documents/documentsSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	documentsRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your documents have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your documents haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		documentsRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		documentsRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			documentsRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your documents have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			documentsRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	documentsRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Documents entity */
app.factory('DocumentsRestService', function ($resource) {
	return $resource('api/documentss/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/documentss/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/documentss/:id'},
		'update': { method:'PUT', url: 'api/documentss/:id'},
		'delete': { method:'DELETE', url: 'api/documentss/:id' },
		'search': { method: 'POST', url: 'api/documentss/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on Documents entity */
app.factory('DocumentsRestSearchService', function ($resource) {
	return $resource('api/documentss/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Documents entity */
app.factory('DocumentsRestIndexService', function ($resource) {
	return $resource('api/documentss/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Documents entity */
app.factory('DocumentsRestMassDeleteService', function ($resource) {
	return $resource('api/documentss/mass/:id', {}, {
...
app.factory('DocumentsRestInvRelationService', function ($resource) {
