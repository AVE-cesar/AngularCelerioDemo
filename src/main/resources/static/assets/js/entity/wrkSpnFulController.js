...
app.controller("WrkSpnFulController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "WrkSpnFulRestService", "WrkSpnFulRestSearchService", 
		"WrkSpnFulRestIndexService", "WrkSpnFulRestMassDeleteService",
		"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		wrkSpnFulRestService, wrkSpnFulRestSearchService, wrkSpnFulRestIndexService, 
		wrkSpnFulRestMassDeleteService, 
		alertService, timeoutService, config) {

log.info("inside WrkSpnFulController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside WrkSpnFulController");
	wrkSpnFulRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside WrkSpnFulController");
	wrkSpnFulRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	wrkSpnFulRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside WrkSpnFulController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside WrkSpnFulController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside WrkSpnFulController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside WrkSpnFulController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside WrkSpnFulController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside WrkSpnFulController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	wrkSpnFulRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	wrkSpnFulRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-wrkSpnFul.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/wrkSpnFul/wrkSpnFulSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	wrkSpnFulRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			wrkSpnFulRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your wrkSpnFul have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			wrkSpnFulRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	wrkSpnFulRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
						...
						/** main REST client for managing (4 CRUD calls) WrkSpnFul entity */
app.factory('WrkSpnFulRestService', function ($resource) {
	return $resource('api/wrkSpnFuls/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/wrkSpnFuls/:wrkSpnIde,:wrkAppCde,:wrkDfuDte',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/wrkSpnFuls/:wrkSpnIde,:wrkAppCde,:wrkDfuDte'},
		'update': { method:'PUT', url: 'api/wrkSpnFuls/:wrkSpnIde,:wrkAppCde,:wrkDfuDte'},
		'delete': { method:'DELETE', url: 'api/wrkSpnFuls/:wrkSpnIde,:wrkAppCde,:wrkDfuDte' },
		'search': { method: 'POST', url: 'api/wrkSpnFuls/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on WrkSpnFul entity */
app.factory('WrkSpnFulRestSearchService', function ($resource) {
	return $resource('api/wrkSpnFuls/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing WrkSpnFul entity */
app.factory('WrkSpnFulRestIndexService', function ($resource) {
	return $resource('api/wrkSpnFuls/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on WrkSpnFul entity */
app.factory('WrkSpnFulRestMassDeleteService', function ($resource) {
	return $resource('api/wrkSpnFuls/mass/:id', {}, {
...
app.factory('WrkSpnFulRestInvRelationService', function ($resource) {
