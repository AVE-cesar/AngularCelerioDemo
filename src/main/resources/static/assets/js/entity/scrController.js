...
app.controller("ScrController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "ScrRestService", "ScrRestSearchService", 
		"ScrRestIndexService", "ScrRestMassDeleteService",
									"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		scrRestService, scrRestSearchService, scrRestIndexService, 
		scrRestMassDeleteService, 
									alertService, timeoutService, config) {

log.info("inside ScrController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside ScrController");
	scrRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside ScrController");
	scrRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	scrRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside ScrController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside ScrController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside ScrController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside ScrController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside ScrController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside ScrController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	scrRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	scrRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-scr.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/scr/scrSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	scrRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your scr have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your scr haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		scrRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		scrRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			scrRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your scr have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			scrRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Scr entity */
app.factory('ScrRestService', function ($resource) {
	return $resource('api/scrs/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/scrs/:id'},
		'update': { method:'PUT', url: 'api/scrs/:id'},
		'delete': { method:'DELETE', url: 'api/scrs/:id' },
		'search': { method: 'POST', url: 'api/scrs/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on Scr entity */
app.factory('ScrRestSearchService', function ($resource) {
	return $resource('api/scrs/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Scr entity */
app.factory('ScrRestIndexService', function ($resource) {
	return $resource('api/scrs/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Scr entity */
app.factory('ScrRestMassDeleteService', function ($resource) {
	return $resource('api/scrs/mass/:id', {}, {
...
app.factory('ScrRestInvRelationService', function ($resource) {
