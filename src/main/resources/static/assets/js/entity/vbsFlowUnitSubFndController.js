...
app.controller("VbsFlowUnitSubFndController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "VbsFlowUnitSubFndRestService", "VbsFlowUnitSubFndRestSearchService", 
		"VbsFlowUnitSubFndRestIndexService", "VbsFlowUnitSubFndRestMassDeleteService",
																			"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		vbsFlowUnitSubFndRestService, vbsFlowUnitSubFndRestSearchService, vbsFlowUnitSubFndRestIndexService, 
		vbsFlowUnitSubFndRestMassDeleteService, 
																			alertService, timeoutService, config) {

log.info("inside VbsFlowUnitSubFndController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside VbsFlowUnitSubFndController");
	vbsFlowUnitSubFndRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside VbsFlowUnitSubFndController");
	vbsFlowUnitSubFndRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	vbsFlowUnitSubFndRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside VbsFlowUnitSubFndController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside VbsFlowUnitSubFndController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside VbsFlowUnitSubFndController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside VbsFlowUnitSubFndController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside VbsFlowUnitSubFndController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside VbsFlowUnitSubFndController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	vbsFlowUnitSubFndRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	vbsFlowUnitSubFndRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-vbsFlowUnitSubFnd.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/vbsFlowUnitSubFnd/vbsFlowUnitSubFndSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	vbsFlowUnitSubFndRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your vbsFlowUnitSubFnd have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your vbsFlowUnitSubFnd haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		vbsFlowUnitSubFndRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		vbsFlowUnitSubFndRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			vbsFlowUnitSubFndRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your vbsFlowUnitSubFnd have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			vbsFlowUnitSubFndRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	vbsFlowUnitSubFndRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) VbsFlowUnitSubFnd entity */
app.factory('VbsFlowUnitSubFndRestService', function ($resource) {
	return $resource('api/vbsFlowUnitSubFnds/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/vbsFlowUnitSubFnds/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/vbsFlowUnitSubFnds/:id'},
		'update': { method:'PUT', url: 'api/vbsFlowUnitSubFnds/:id'},
		'delete': { method:'DELETE', url: 'api/vbsFlowUnitSubFnds/:id' },
		'search': { method: 'POST', url: 'api/vbsFlowUnitSubFnds/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on VbsFlowUnitSubFnd entity */
app.factory('VbsFlowUnitSubFndRestSearchService', function ($resource) {
	return $resource('api/vbsFlowUnitSubFnds/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing VbsFlowUnitSubFnd entity */
app.factory('VbsFlowUnitSubFndRestIndexService', function ($resource) {
	return $resource('api/vbsFlowUnitSubFnds/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on VbsFlowUnitSubFnd entity */
app.factory('VbsFlowUnitSubFndRestMassDeleteService', function ($resource) {
	return $resource('api/vbsFlowUnitSubFnds/mass/:id', {}, {
...
app.factory('VbsFlowUnitSubFndRestInvRelationService', function ($resource) {
