...
app.controller("VbsFlowUnitDiffusionController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "VbsFlowUnitDiffusionRestService", "VbsFlowUnitDiffusionRestSearchService", 
		"VbsFlowUnitDiffusionRestIndexService", "VbsFlowUnitDiffusionRestMassDeleteService",
					"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		vbsFlowUnitDiffusionRestService, vbsFlowUnitDiffusionRestSearchService, vbsFlowUnitDiffusionRestIndexService, 
		vbsFlowUnitDiffusionRestMassDeleteService, 
					alertService, timeoutService, config) {

log.info("inside VbsFlowUnitDiffusionController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside VbsFlowUnitDiffusionController");
	vbsFlowUnitDiffusionRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside VbsFlowUnitDiffusionController");
	vbsFlowUnitDiffusionRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	vbsFlowUnitDiffusionRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside VbsFlowUnitDiffusionController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside VbsFlowUnitDiffusionController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside VbsFlowUnitDiffusionController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside VbsFlowUnitDiffusionController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside VbsFlowUnitDiffusionController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside VbsFlowUnitDiffusionController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	vbsFlowUnitDiffusionRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	vbsFlowUnitDiffusionRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-vbsFlowUnitDiffusion.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/vbsFlowUnitDiffusion/vbsFlowUnitDiffusionSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	vbsFlowUnitDiffusionRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your vbsFlowUnitDiffusion have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your vbsFlowUnitDiffusion haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		vbsFlowUnitDiffusionRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		vbsFlowUnitDiffusionRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			vbsFlowUnitDiffusionRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your vbsFlowUnitDiffusion have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			vbsFlowUnitDiffusionRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	vbsFlowUnitDiffusionRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) VbsFlowUnitDiffusion entity */
app.factory('VbsFlowUnitDiffusionRestService', function ($resource) {
	return $resource('api/vbsFlowUnitDiffusions/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/vbsFlowUnitDiffusions/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/vbsFlowUnitDiffusions/:id'},
		'update': { method:'PUT', url: 'api/vbsFlowUnitDiffusions/:id'},
		'delete': { method:'DELETE', url: 'api/vbsFlowUnitDiffusions/:id' },
		'search': { method: 'POST', url: 'api/vbsFlowUnitDiffusions/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on VbsFlowUnitDiffusion entity */
app.factory('VbsFlowUnitDiffusionRestSearchService', function ($resource) {
	return $resource('api/vbsFlowUnitDiffusions/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing VbsFlowUnitDiffusion entity */
app.factory('VbsFlowUnitDiffusionRestIndexService', function ($resource) {
	return $resource('api/vbsFlowUnitDiffusions/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on VbsFlowUnitDiffusion entity */
app.factory('VbsFlowUnitDiffusionRestMassDeleteService', function ($resource) {
	return $resource('api/vbsFlowUnitDiffusions/mass/:id', {}, {
...
app.factory('VbsFlowUnitDiffusionRestInvRelationService', function ($resource) {
