...
app.controller("VbsStatusController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "VbsStatusRestService", "VbsStatusRestSearchService", 
		"VbsStatusRestIndexService", "VbsStatusRestMassDeleteService",
												"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		vbsStatusRestService, vbsStatusRestSearchService, vbsStatusRestIndexService, 
		vbsStatusRestMassDeleteService, 
												alertService, timeoutService, config) {

log.info("inside VbsStatusController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside VbsStatusController");
	vbsStatusRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside VbsStatusController");
	vbsStatusRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	vbsStatusRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside VbsStatusController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside VbsStatusController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside VbsStatusController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside VbsStatusController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside VbsStatusController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside VbsStatusController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	vbsStatusRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	vbsStatusRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-vbsStatus.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/vbsStatus/vbsStatusSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	vbsStatusRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your vbsStatus have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your vbsStatus haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		vbsStatusRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		vbsStatusRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			vbsStatusRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your vbsStatus have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			vbsStatusRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	vbsStatusRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) VbsStatus entity */
app.factory('VbsStatusRestService', function ($resource) {
	return $resource('api/vbsStatuss/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/vbsStatuss/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/vbsStatuss/:id'},
		'update': { method:'PUT', url: 'api/vbsStatuss/:id'},
		'delete': { method:'DELETE', url: 'api/vbsStatuss/:id' },
		'search': { method: 'POST', url: 'api/vbsStatuss/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on VbsStatus entity */
app.factory('VbsStatusRestSearchService', function ($resource) {
	return $resource('api/vbsStatuss/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing VbsStatus entity */
app.factory('VbsStatusRestIndexService', function ($resource) {
	return $resource('api/vbsStatuss/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on VbsStatus entity */
app.factory('VbsStatusRestMassDeleteService', function ($resource) {
	return $resource('api/vbsStatuss/mass/:id', {}, {
...
app.factory('VbsStatusRestInvRelationService', function ($resource) {
