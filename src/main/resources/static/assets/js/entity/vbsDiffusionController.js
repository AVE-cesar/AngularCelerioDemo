...
app.controller("VbsDiffusionController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "VbsDiffusionRestService", "VbsDiffusionRestSearchService", 
		"VbsDiffusionRestIndexService", "VbsDiffusionRestMassDeleteService",
																					"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		vbsDiffusionRestService, vbsDiffusionRestSearchService, vbsDiffusionRestIndexService, 
		vbsDiffusionRestMassDeleteService, 
																					alertService, timeoutService, config) {

log.info("inside VbsDiffusionController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside VbsDiffusionController");
	vbsDiffusionRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside VbsDiffusionController");
	vbsDiffusionRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	vbsDiffusionRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside VbsDiffusionController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside VbsDiffusionController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside VbsDiffusionController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside VbsDiffusionController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside VbsDiffusionController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside VbsDiffusionController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	vbsDiffusionRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	vbsDiffusionRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-vbsDiffusion.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/vbsDiffusion/vbsDiffusionSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	vbsDiffusionRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your vbsDiffusion have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your vbsDiffusion haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		vbsDiffusionRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		vbsDiffusionRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			vbsDiffusionRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your vbsDiffusion have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			vbsDiffusionRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	vbsDiffusionRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) VbsDiffusion entity */
app.factory('VbsDiffusionRestService', function ($resource) {
	return $resource('api/vbsDiffusions/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/vbsDiffusions/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/vbsDiffusions/:id'},
		'update': { method:'PUT', url: 'api/vbsDiffusions/:id'},
		'delete': { method:'DELETE', url: 'api/vbsDiffusions/:id' },
		'search': { method: 'POST', url: 'api/vbsDiffusions/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on VbsDiffusion entity */
app.factory('VbsDiffusionRestSearchService', function ($resource) {
	return $resource('api/vbsDiffusions/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing VbsDiffusion entity */
app.factory('VbsDiffusionRestIndexService', function ($resource) {
	return $resource('api/vbsDiffusions/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on VbsDiffusion entity */
app.factory('VbsDiffusionRestMassDeleteService', function ($resource) {
	return $resource('api/vbsDiffusions/mass/:id', {}, {
...
app.factory('VbsDiffusionRestInvRelationService', function ($resource) {
