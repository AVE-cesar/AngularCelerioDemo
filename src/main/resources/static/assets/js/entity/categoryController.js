...
app.controller("CategoryController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "CategoryRestService", "CategoryRestSearchService", 
		"CategoryRestIndexService", "CategoryRestMassDeleteService",
				"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		categoryRestService, categoryRestSearchService, categoryRestIndexService, 
		categoryRestMassDeleteService, 
				alertService, timeoutService, config) {

log.info("inside CategoryController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside CategoryController");
	categoryRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside CategoryController");
	categoryRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	categoryRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside CategoryController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside CategoryController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside CategoryController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside CategoryController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside CategoryController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside CategoryController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	categoryRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	categoryRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-category.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/category/categorySearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	categoryRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your category have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your category haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		categoryRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		categoryRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			categoryRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your category have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			categoryRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	categoryRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Category entity */
app.factory('CategoryRestService', function ($resource) {
	return $resource('api/categories/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/categories/:id'},
		'update': { method:'PUT', url: 'api/categories/:id'},
		'delete': { method:'DELETE', url: 'api/categories/:id' },
		'search': { method: 'POST', url: 'api/categories/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on Category entity */
app.factory('CategoryRestSearchService', function ($resource) {
	return $resource('api/categories/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Category entity */
app.factory('CategoryRestIndexService', function ($resource) {
	return $resource('api/categories/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Category entity */
app.factory('CategoryRestMassDeleteService', function ($resource) {
	return $resource('api/categories/mass/:id', {}, {
...
app.factory('CategoryRestInvRelationService', function ($resource) {
	return $resource('api/void/:id', {}, {
		'void': { method: 'GET'} /* dummy method, to be sure to have at least one */

                ,'findParameterByCategory': { method: 'GET', isArray: true, url: 'api/parameters/findByCategory/:id'}
      	});
});
