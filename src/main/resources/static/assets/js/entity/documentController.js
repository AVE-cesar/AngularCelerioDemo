...
app.controller("DocumentController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "DocumentRestService", "DocumentRestSearchService", 
		"DocumentRestIndexService", "DocumentRestMassDeleteService",
				"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		documentRestService, documentRestSearchService, documentRestIndexService, 
		documentRestMassDeleteService, 
				alertService, timeoutService, config) {

log.info("inside DocumentController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside DocumentController");
	documentRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside DocumentController");
	documentRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	documentRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside DocumentController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside DocumentController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside DocumentController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside DocumentController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside DocumentController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside DocumentController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	documentRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	documentRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-document.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/document/documentSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	documentRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your document have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	if (scope.item.id != null) {
		...
		// creation mode
		documentRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			documentRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your document have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
...
scope.index = function() {
	documentRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Document entity */
app.factory('DocumentRestService', function ($resource) {
	return $resource('api/documents/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/documents/:id'},
		'update': { method:'PUT', url: 'api/documents/:id'},
		'delete': { method:'DELETE', url: 'api/documents/:id' },
		'search': { method: 'POST', url: 'api/documents/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on Document entity */
app.factory('DocumentRestSearchService', function ($resource) {
	return $resource('api/documents/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Document entity */
app.factory('DocumentRestIndexService', function ($resource) {
	return $resource('api/documents/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Document entity */
app.factory('DocumentRestMassDeleteService', function ($resource) {
	return $resource('api/documents/mass/:id', {}, {
...
app.factory('DocumentRestInvRelationService', function ($resource) {
