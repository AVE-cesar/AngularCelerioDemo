...
app.controller("AppTranslationController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "AppTranslationRestService", "AppTranslationRestSearchService", 
		"AppTranslationRestIndexService", "AppTranslationRestMassDeleteService",
						"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		appTranslationRestService, appTranslationRestSearchService, appTranslationRestIndexService, 
		appTranslationRestMassDeleteService, 
						alertService, timeoutService, config) {

log.info("inside AppTranslationController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside AppTranslationController");
	appTranslationRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside AppTranslationController");
	appTranslationRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	appTranslationRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside AppTranslationController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside AppTranslationController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside AppTranslationController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside AppTranslationController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside AppTranslationController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside AppTranslationController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	appTranslationRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	appTranslationRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-appTranslation.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/appTranslation/appTranslationSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	appTranslationRestService.get({id: item.id}, function success(result) {
		scope.item = result;
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your appTranslation have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			appTranslationRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your appTranslation have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
...
scope.index = function() {
	appTranslationRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) AppTranslation entity */
app.factory('AppTranslationRestService', function ($resource) {
	return $resource('api/appTranslations/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/appTranslations/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/appTranslations/:id'},
		'update': { method:'PUT', url: 'api/appTranslations/:id'},
		'delete': { method:'DELETE', url: 'api/appTranslations/:id' },
		'search': { method: 'POST', url: 'api/appTranslations/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on AppTranslation entity */
app.factory('AppTranslationRestSearchService', function ($resource) {
	return $resource('api/appTranslations/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing AppTranslation entity */
app.factory('AppTranslationRestIndexService', function ($resource) {
	return $resource('api/appTranslations/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on AppTranslation entity */
app.factory('AppTranslationRestMassDeleteService', function ($resource) {
	return $resource('api/appTranslations/mass/:id', {}, {
...
app.factory('AppTranslationRestInvRelationService', function ($resource) {
