...
app.controller("AuthorStyleController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "AuthorStyleRestService", "AuthorStyleRestSearchService", 
		"AuthorStyleRestIndexService", "AuthorStyleRestMassDeleteService",
							"AuthorRestService",
									"StyleRestService",
						"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		authorStyleRestService, authorStyleRestSearchService, authorStyleRestIndexService, 
		authorStyleRestMassDeleteService, 
							authorRestService,
									styleRestService,
						alertService, timeoutService, config) {

log.info("inside AuthorStyleController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside AuthorStyleController");
	authorStyleRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside AuthorStyleController");
	authorStyleRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	authorStyleRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside AuthorStyleController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside AuthorStyleController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside AuthorStyleController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside AuthorStyleController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside AuthorStyleController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside AuthorStyleController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	authorStyleRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	authorStyleRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-authorStyle.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/authorStyle/authorStyleSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	authorStyleRestService.get({id: item.id}, function success(result) {
		scope.item = result;
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your authorStyle have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			authorStyleRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your authorStyle have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			authorStyleRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	authorStyleRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) AuthorStyle entity */
app.factory('AuthorStyleRestService', function ($resource) {
	return $resource('api/authorStyles/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/authorStyles/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/authorStyles/:id'},
		'update': { method:'PUT', url: 'api/authorStyles/:id'},
		'delete': { method:'DELETE', url: 'api/authorStyles/:id' },
		'search': { method: 'POST', url: 'api/authorStyles/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on AuthorStyle entity */
app.factory('AuthorStyleRestSearchService', function ($resource) {
	return $resource('api/authorStyles/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing AuthorStyle entity */
app.factory('AuthorStyleRestIndexService', function ($resource) {
	return $resource('api/authorStyles/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on AuthorStyle entity */
app.factory('AuthorStyleRestMassDeleteService', function ($resource) {
	return $resource('api/authorStyles/mass/:id', {}, {
...
app.factory('AuthorStyleRestInvRelationService', function ($resource) {
