...
app.controller("VbsFlowController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "VbsFlowRestService", "VbsFlowRestSearchService", 
		"VbsFlowRestIndexService", "VbsFlowRestMassDeleteService",
													"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		vbsFlowRestService, vbsFlowRestSearchService, vbsFlowRestIndexService, 
		vbsFlowRestMassDeleteService, 
													alertService, timeoutService, config) {

log.info("inside VbsFlowController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside VbsFlowController");
	vbsFlowRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside VbsFlowController");
	vbsFlowRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	vbsFlowRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside VbsFlowController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside VbsFlowController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside VbsFlowController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside VbsFlowController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside VbsFlowController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside VbsFlowController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	vbsFlowRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	vbsFlowRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-vbsFlow.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/vbsFlow/vbsFlowSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	vbsFlowRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your vbsFlow have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your vbsFlow haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		vbsFlowRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		vbsFlowRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			vbsFlowRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your vbsFlow have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			vbsFlowRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) VbsFlow entity */
app.factory('VbsFlowRestService', function ($resource) {
	return $resource('api/vbsFlows/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/vbsFlows/:id'},
		'update': { method:'PUT', url: 'api/vbsFlows/:id'},
		'delete': { method:'DELETE', url: 'api/vbsFlows/:id' },
		'search': { method: 'POST', url: 'api/vbsFlows/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on VbsFlow entity */
app.factory('VbsFlowRestSearchService', function ($resource) {
	return $resource('api/vbsFlows/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing VbsFlow entity */
app.factory('VbsFlowRestIndexService', function ($resource) {
	return $resource('api/vbsFlows/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on VbsFlow entity */
app.factory('VbsFlowRestMassDeleteService', function ($resource) {
	return $resource('api/vbsFlows/mass/:id', {}, {
...
app.factory('VbsFlowRestInvRelationService', function ($resource) {
