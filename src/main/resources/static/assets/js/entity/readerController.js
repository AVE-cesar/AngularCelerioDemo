...
app.controller("ReaderController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "ReaderRestService", "ReaderRestSearchService", 
		"ReaderRestIndexService", "ReaderRestMassDeleteService",
					"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		readerRestService, readerRestSearchService, readerRestIndexService, 
		readerRestMassDeleteService, 
					alertService, timeoutService, config) {

log.info("inside ReaderController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside ReaderController");
	readerRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside ReaderController");
	readerRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	readerRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside ReaderController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside ReaderController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside ReaderController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside ReaderController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside ReaderController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside ReaderController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	readerRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	readerRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-reader.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/reader/readerSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	readerRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your reader have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	if (scope.item.id != null) {
		...
		// creation mode
		readerRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			readerRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your reader have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Reader entity */
app.factory('ReaderRestService', function ($resource) {
	return $resource('api/readers/bypage/?page=:page&size=:size', {}, {
		...
		'create': { method:'POST', url: 'api/readers/:id'},
		'update': { method:'PUT', url: 'api/readers/:id'},
		'delete': { method:'DELETE', url: 'api/readers/:id' },
		'search': { method: 'POST', url: 'api/readers/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on Reader entity */
app.factory('ReaderRestSearchService', function ($resource) {
	return $resource('api/readers/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Reader entity */
app.factory('ReaderRestIndexService', function ($resource) {
	return $resource('api/readers/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Reader entity */
app.factory('ReaderRestMassDeleteService', function ($resource) {
	return $resource('api/readers/mass/:id', {}, {
...
app.factory('ReaderRestInvRelationService', function ($resource) {
	return $resource('api/void/:id', {}, {
		'void': { method: 'GET'} /* dummy method, to be sure to have at least one */

        ,'findBookByReader': { method: 'GET', isArray: true, url: 'api/books/findByReader/:id'}
              	});
});
