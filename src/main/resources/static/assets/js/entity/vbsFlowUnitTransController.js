...
app.controller("VbsFlowUnitTransController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "VbsFlowUnitTransRestService", "VbsFlowUnitTransRestSearchService", 
		"VbsFlowUnitTransRestIndexService", "VbsFlowUnitTransRestMassDeleteService",
					"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		vbsFlowUnitTransRestService, vbsFlowUnitTransRestSearchService, vbsFlowUnitTransRestIndexService, 
		vbsFlowUnitTransRestMassDeleteService, 
					alertService, timeoutService, config) {

log.info("inside VbsFlowUnitTransController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside VbsFlowUnitTransController");
	vbsFlowUnitTransRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside VbsFlowUnitTransController");
	vbsFlowUnitTransRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	vbsFlowUnitTransRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside VbsFlowUnitTransController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside VbsFlowUnitTransController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside VbsFlowUnitTransController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside VbsFlowUnitTransController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside VbsFlowUnitTransController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside VbsFlowUnitTransController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	vbsFlowUnitTransRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	vbsFlowUnitTransRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-vbsFlowUnitTrans.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/vbsFlowUnitTrans/vbsFlowUnitTransSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	vbsFlowUnitTransRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			vbsFlowUnitTransRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your vbsFlowUnitTrans have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
...
scope.index = function() {
	vbsFlowUnitTransRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) VbsFlowUnitTrans entity */
app.factory('VbsFlowUnitTransRestService', function ($resource) {
	return $resource('api/vbsFlowUnitTranss/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/vbsFlowUnitTranss/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/vbsFlowUnitTranss/:id'},
		'update': { method:'PUT', url: 'api/vbsFlowUnitTranss/:id'},
		'delete': { method:'DELETE', url: 'api/vbsFlowUnitTranss/:id' },
		'search': { method: 'POST', url: 'api/vbsFlowUnitTranss/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on VbsFlowUnitTrans entity */
app.factory('VbsFlowUnitTransRestSearchService', function ($resource) {
	return $resource('api/vbsFlowUnitTranss/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing VbsFlowUnitTrans entity */
app.factory('VbsFlowUnitTransRestIndexService', function ($resource) {
	return $resource('api/vbsFlowUnitTranss/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on VbsFlowUnitTrans entity */
app.factory('VbsFlowUnitTransRestMassDeleteService', function ($resource) {
	return $resource('api/vbsFlowUnitTranss/mass/:id', {}, {
...
app.factory('VbsFlowUnitTransRestInvRelationService', function ($resource) {
