...
app.controller("ParameterController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "ParameterRestService", "ParameterRestSearchService", 
		"ParameterRestIndexService", "ParameterRestMassDeleteService",
				"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		parameterRestService, parameterRestSearchService, parameterRestIndexService, 
		parameterRestMassDeleteService, 
				alertService, timeoutService, config) {

log.info("inside ParameterController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside ParameterController");
	parameterRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside ParameterController");
	parameterRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	parameterRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside ParameterController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside ParameterController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside ParameterController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside ParameterController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside ParameterController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside ParameterController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	parameterRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	parameterRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-parameter.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/parameter/parameterSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	parameterRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your parameter have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your parameter haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		parameterRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		parameterRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			parameterRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your parameter have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			parameterRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	parameterRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
						...
						/** main REST client for managing (4 CRUD calls) Parameter entity */
app.factory('ParameterRestService', function ($resource) {
	return $resource('api/parameters/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/parameters/:parCatIde,:key',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/parameters/:parCatIde,:key'},
		'update': { method:'PUT', url: 'api/parameters/:parCatIde,:key'},
		'delete': { method:'DELETE', url: 'api/parameters/:parCatIde,:key' },
		'search': { method: 'POST', url: 'api/parameters/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on Parameter entity */
app.factory('ParameterRestSearchService', function ($resource) {
	return $resource('api/parameters/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Parameter entity */
app.factory('ParameterRestIndexService', function ($resource) {
	return $resource('api/parameters/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Parameter entity */
app.factory('ParameterRestMassDeleteService', function ($resource) {
	return $resource('api/parameters/mass/:id', {}, {
...
app.factory('ParameterRestInvRelationService', function ($resource) {
