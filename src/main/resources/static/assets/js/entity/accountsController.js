...
app.controller("AccountsController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "AccountsRestService", "AccountsRestSearchService", 
		"AccountsRestIndexService", "AccountsRestMassDeleteService",
							"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		accountsRestService, accountsRestSearchService, accountsRestIndexService, 
		accountsRestMassDeleteService, 
							alertService, timeoutService, config) {

log.info("inside AccountsController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside AccountsController");
	accountsRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside AccountsController");
	accountsRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	accountsRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside AccountsController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside AccountsController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside AccountsController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside AccountsController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside AccountsController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside AccountsController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	accountsRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	accountsRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-accounts.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/accounts/accountsSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	accountsRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your accounts have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your accounts haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		accountsRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		accountsRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			accountsRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your accounts have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			accountsRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	accountsRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Accounts entity */
app.factory('AccountsRestService', function ($resource) {
	return $resource('api/accountss/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/accountss/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/accountss/:id'},
		'update': { method:'PUT', url: 'api/accountss/:id'},
		'delete': { method:'DELETE', url: 'api/accountss/:id' },
		'search': { method: 'POST', url: 'api/accountss/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on Accounts entity */
app.factory('AccountsRestSearchService', function ($resource) {
	return $resource('api/accountss/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Accounts entity */
app.factory('AccountsRestIndexService', function ($resource) {
	return $resource('api/accountss/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Accounts entity */
app.factory('AccountsRestMassDeleteService', function ($resource) {
	return $resource('api/accountss/mass/:id', {}, {
...
app.factory('AccountsRestInvRelationService', function ($resource) {
	return $resource('api/void/:id', {}, {
		'void': { method: 'GET'} /* dummy method, to be sure to have at least one */

                     ,'findTransactionsByAccounts': { method: 'GET', isArray: true, url: 'api/transactionss/findByAccounts/:id'}
        ,'findTransactionsByAccounts': { method: 'GET', isArray: true, url: 'api/transactionss/findByAccounts/:id'}
   	});
});
