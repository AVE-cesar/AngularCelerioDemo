...
app.controller("VbsFlowUnitController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "VbsFlowUnitRestService", "VbsFlowUnitRestSearchService", 
		"VbsFlowUnitRestIndexService", "VbsFlowUnitRestMassDeleteService",
																																																									"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		vbsFlowUnitRestService, vbsFlowUnitRestSearchService, vbsFlowUnitRestIndexService, 
		vbsFlowUnitRestMassDeleteService, 
																																																									alertService, timeoutService, config) {

log.info("inside VbsFlowUnitController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside VbsFlowUnitController");
	vbsFlowUnitRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside VbsFlowUnitController");
	vbsFlowUnitRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	vbsFlowUnitRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside VbsFlowUnitController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside VbsFlowUnitController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside VbsFlowUnitController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside VbsFlowUnitController for page: " + (scope.pagination.totalPages - 1));
...
scope.clear = function () {
	log.info("call method clear inside VbsFlowUnitController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside VbsFlowUnitController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	vbsFlowUnitRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	vbsFlowUnitRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-vbsFlowUnit.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/vbsFlowUnit/vbsFlowUnitSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	vbsFlowUnitRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your vbsFlowUnit have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your vbsFlowUnit haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		vbsFlowUnitRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		vbsFlowUnitRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			vbsFlowUnitRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your vbsFlowUnit have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			vbsFlowUnitRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	vbsFlowUnitRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) VbsFlowUnit entity */
app.factory('VbsFlowUnitRestService', function ($resource) {
	return $resource('api/vbsFlowUnits/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/vbsFlowUnits/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/vbsFlowUnits/:id'},
		'update': { method:'PUT', url: 'api/vbsFlowUnits/:id'},
		'delete': { method:'DELETE', url: 'api/vbsFlowUnits/:id' },
		'search': { method: 'POST', url: 'api/vbsFlowUnits/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on VbsFlowUnit entity */
app.factory('VbsFlowUnitRestSearchService', function ($resource) {
	return $resource('api/vbsFlowUnits/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing VbsFlowUnit entity */
app.factory('VbsFlowUnitRestIndexService', function ($resource) {
	return $resource('api/vbsFlowUnits/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on VbsFlowUnit entity */
app.factory('VbsFlowUnitRestMassDeleteService', function ($resource) {
	return $resource('api/vbsFlowUnits/mass/:id', {}, {
...
app.factory('VbsFlowUnitRestInvRelationService', function ($resource) {
