...
app.controller("TransactionsController", ["$scope", "$window", "$aside", "PlaceholderTextService", 
"$log", "TransactionsRestService", "TransactionsRestSearchService", 
		"TransactionsRestIndexService", "TransactionsRestMassDeleteService",
							"AccountsRestService",
									"AccountsRestService",
								"$alert", "$timeout", "config", function(scope, window, c, d, log, 
		transactionsRestService, transactionsRestSearchService, transactionsRestIndexService, 
		transactionsRestMassDeleteService, 
							accountsRestService,
									accountsRestService,
								alertService, timeoutService, config) {

log.info("inside TransactionsController, config.value: " + config.value);
...
scope.refresh = function () {
	log.info("call method refresh inside TransactionsController");
	transactionsRestService.query({page: 0, size: scope.totalElementsPerPage}, function(result) {
...
scope.refreshByPage = function (page, size, addMode) {
	log.info("call method refreshByPage inside TransactionsController");
	transactionsRestService.query({page: page, size: size}, function(result) {
...
scope.startSearch = function(item) {
	...
	transactionsRestService.search(item, function success(result){
		log.info("receiving info from server side");
...
scope.first = function () {
	log.info("call method first inside TransactionsController for page: 0");
	scope.refreshByPage(0, scope.totalElementsPerPage, false);		
}; 

/** Gets previous page */
scope.prev = function () {
	log.info("call method prev inside TransactionsController for page: " + (scope.pagination.number - 1));
	scope.refreshByPage(scope.pagination.number - 1, scope.totalElementsPerPage, false);		
}; 

/** Gets next page */
scope.next = function () {
	log.info("call method next inside TransactionsController for page: " + (scope.pagination.number + 1));
	scope.refreshByPage(scope.pagination.number + 1, scope.totalElementsPerPage, false);		
}; 

/** Gets last page */
scope.last = function () {
	log.info("call method last inside TransactionsController for page: " + (scope.pagination.totalPages - 1));
	scope.refreshByPage(scope.pagination.totalPages - 1, scope.totalElementsPerPage, false);		
};

				// fill Accounts combo with data from server side
accountsRestService.query({query: '*'}, function success(result){
	log.info("receiving Accounts from server side");
	scope.accountss = result;
	log.info("Accounts post refresh: " + result.length);
});
						// fill Accounts combo with data from server side
accountsRestService.query({query: '*'}, function success(result){
	log.info("receiving Accounts from server side");
...
scope.clear = function () {
	log.info("call method clear inside TransactionsController");
...
scope.clearSavedSearch = function () {
	log.info("call method clearSavedSearch inside TransactionsController");
	scope.savedSearch = {};
};
...
scope.startElasticSearch = function(item) {
	...
	transactionsRestSearchService.query({query: query}, function success(result){
		log.info("receiving info from server side");
...
scope.loadOneItem = function(id) {
	transactionsRestService.get({id: id}, function success(result) {
		scope.item = result;
...
var crudAside = c({
	scope: scope,
	template: "assets/tpl/apps/crud-transactions.html",
	show: !1,
	placement: "left",
	backdrop: !1,
...
var searchAside = c({
	scope: scope,
	template: "assets/tpl/apps/transactions/transactionsSearch.html",
	show: !1,
	placement: "left",
	backdrop: !1,
	animation: "am-slide-left"
...
scope.editItem = function(item) {
	...
	transactionsRestService.get({id: item.id}, function success(result) {
		scope.item = result;
...
scope.saveItem = function() {
	...
	var onSaveSuccess = function success(data) {
		...
		window.showAlert = function(){
			...
			var userALert = alertService({
	                    title: "SUCCESS:",
	                    content: '<BR>Your transactions have been <i>created or updated</i>. You can find it int the result table. See <a href="#"><B>older messages</B></a> !',
	                    placement: "top-right",
	                    type: "theme",
	...
	var onSaveError = function (result) {
			window.showAlert = function(){
				var userALert = alertService({
		                    title: "ERROR:",
		                    content: "<BR>Your transactions haven't been created. Try again !",
		                    placement: "top-right",
		                    type: "theme",
		                    container: ".alert-container-top-right",
		                    show: !1,
	...
	if (scope.item.id != null) {
		// update mode
		transactionsRestService.update(scope.item, onSaveSuccess, onSaveError);
	} else {
		// creation mode
		transactionsRestService.save(scope.item, onSaveSuccess, onSaveError);
	}
};
...
scope.remove = function(b) {
	...
	if (r == true) {/*
		...
		if (!b) {
			...
			transactionsRestMassDeleteService.massDelete({id: ids}, function success(data) {
				scope.refresh();
				
				window.showAlert = function(){
				    var userALert = alertService({
				                    title: "SUCCESS:",
				                    content: '<BR>Your transactions have been deleted. The result table have been <b>updated</b>.',
				                    placement: "top-right",
				                    type: "theme",
				                    container: ".alert-container-top-right",
				                    show: !1,
			...
			transactionsRestService.delete({id: b.id}, function success(data) {
				scope.refresh();
			}, function failure(err) {
				alert('request failed');
			});
...
scope.index = function() {
	transactionsRestIndexService.index();
};
...
scope.$on("$destroy", function() {
	...
	hideForm(searchAside);
				...
				/** main REST client for managing (4 CRUD calls) Transactions entity */
app.factory('TransactionsRestService', function ($resource) {
	return $resource('api/transactionss/bypage/?page=:page&size=:size', {}, {
			/* sorting sample: &sort=aColumnName,desc&sort=anotherColumnName,asc */
		'query': { method: 'GET', isArray: false},
		'get': {
			method: 'GET',
			url: 'api/transactionss/:id',
			transformResponse: function (data) {
				data = angular.fromJson(data);
				return data;
			}
		},
		'create': { method:'POST', url: 'api/transactionss/:id'},
		'update': { method:'PUT', url: 'api/transactionss/:id'},
		'delete': { method:'DELETE', url: 'api/transactionss/:id' },
		'search': { method: 'POST', url: 'api/transactionss/search/', isArray: false}
	});
});

/** REST client for managing Elastic search calls on Transactions entity */
app.factory('TransactionsRestSearchService', function ($resource) {
	return $resource('api/transactionss/esearch/:query', {}, {
		'query': { method: 'GET', isArray: true}
	});
});

/** REST client for indexing Transactions entity */
app.factory('TransactionsRestIndexService', function ($resource) {
	return $resource('api/transactionss/indexAll', {}, {
		'index': { method: 'GET'}
	});
});

/** REST client for managing mass deletion calls on Transactions entity */
app.factory('TransactionsRestMassDeleteService', function ($resource) {
	return $resource('api/transactionss/mass/:id', {}, {
...
app.factory('TransactionsRestInvRelationService', function ($resource) {
