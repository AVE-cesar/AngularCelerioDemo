...
app.controller("DocumentEditController", ["$scope", "$window", "$aside", 
"$log", "DocumentRestService", 
		"DocumentRestInvRelationService", "$alert", "$timeout", "item", "mode", function(scope, window, aside, log, 
		documentRestService, 
				documentRestInvRelationService, alertService, timeoutService, item, mode) {
	
	log.info("inside DocumentEditController, mode: " + mode);
	log.info("inside DocumentEditController, item: " + item);
	scope.mode = mode;
	scope.item = item;
		...
		if (r == true) {
			if (item) {				
				// one item deletion mode
												documentRestService.delete({id: item.id}, function success(data) {
					scope.clear();
					alert('item deleted');
